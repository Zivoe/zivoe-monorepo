 ...
 * sign-in until that route can issue the pass and return the visitor. A visitor who still has to
 * onboard does that instead and stays in the dapp; `next` is dropped there.
...
/**
 * The Lighthouse page to return to after sign-in, or undefined when `next` is not one, so `next`
...
/** Carries the Lighthouse return URL on a dapp path so it survives sign-in. */
