 ...
 * one on `chain` accepting `assetAddress`, or the chain's default when none is
 * named. What flows hand to every Centrifuge Module hook: the catalog's chain
 * identity joined with the chain's deployment facts, plus the stable public
 * identity for analytics and Sentry. Throws for a chain the Zivoe Vault is not
 * live on, or an asset it does not accept there.
 */
