...
export function HeroElemet1Component() {
  return (
    <div className="absolute top-[-18%] right-[-12%] -z-20 hidden h-full w-full grid-cols-2 items-end gap-4 lg:grid xl:right-[-10%] 2xl:right-[-8%]">
      <div></div>
...
export function HeroElement2TabletComponent() {
  return (
    <HeroElement2Tablet
      aria-hidden="true"
      className="absolute right-0 bottom-0 -z-10 hidden w-[95%] sm:block lg:hidden"
    />
  );
}
...
export function HeroElement2MobileComponent({ className }: { className?: string }) {
  return (
    <HeroElement2Mobile aria-hidden="true" className={cn('absolute right-0 bottom-0 -z-10 sm:hidden', className)} />
  );
}
